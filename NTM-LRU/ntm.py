...
class NTM(nn.Module):
    ...
    def __init__(self, num_inputs, num_outputs, controller, memory, heads, gamma=0.9):
        ...
        for head in heads:
            if head.is_read_head():
                ...
                init_w_u = torch.ones(self.N)*1/self.N
    ...
    def forward(self, x, prev_state):
        ...
        reads = []
        heads_states = []
        weight_use = [w_u.data for w_u in prev_w_u]
        
        
        i = 0 
        for head, prev_head_state in zip(self.heads, prev_heads_states, prev_w_u):
            
            if head.is_read_head():
                prev_read_weight = prev_head_state
                r, head_state = head(controller_outp)
                reads += [r]
                weight_use[i] = self.gamma * weight_use[i] + head_state.data
                
            else:
                head_state = head(controller_outp, prev_read_weight, prev_w_u)
                weight_use[i] += head_state.data
                i = i + 1
        ...
        weight_use = Variable(weight_use)
