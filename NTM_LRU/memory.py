...
class NTMMemory(nn.Module):
    ...
    def write(self, w, k):
        ...
        for b in range(self.batch_size):
            ...
            add = torch.ger(w[b], k[b])
            self.memory[b] = self.prev_mem[b] + add
    ...
    def address_read(self, k, β):
        wc = self._similarity(k, β)
        return wc
    
    def address_write(self, alpha, w_r_prev, w_lu):
        
        w_w = self._interpolate(w_r_prev, w_lu, alpha)
